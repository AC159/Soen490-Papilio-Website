...
const ProgressBall = ({ icon, text, selected }: IProgressBall): JSX.Element => {
  const style = classNames(
    'material-symbols-outlined whitespace-nowrap bg-white rounded-full p-1.5',
    {
  ...
  const textStyle = classNames(
    'absolute origin-left left-14 top-1/2 -translate-y-1/2 whitespace-nowrap text-xs',
    {
      'text-brand-orange-light': selected,
      'text-white': !selected,
    },
...
const MultiStepForm = ({
  ...
  let onStepChange = async (data: any): Promise<void> => {
    console.log(data);
    setStep((prev) => prev + 1);
  };
  const submit = async (): Promise<void> => await onSubmit(formData);
  let onBack = async (data: any): Promise<void> => {
    console.log(data);
    setStep((prev) => prev - 1);
  };
  ...
  if (steps[step].stepId === 'profile') {
    ...
    Body = (
      <ProfileForm initialState={formData.profile} onSubmit={onStepChange} />
    );
    ...
    Body = (
      <AdminForm
        initialState={formData.adminAccount}
    ...
    <div className="flex flex-row border border-brand-blue-dark w-4/6">
      <div className="bg-brand-blue-dark px-9 flex flex-col items-center pb-9 overflow-hidden w-64">
        <div className="-mt-8 bg-brand-blue-dark">
          <span className="font-display text-ultra text-brand-blue-light-dark">
            {step + 1}
          </span>
        </div>
        <h2 className="pt-6 pb-8 text-2xl font-medium text-center leading text-white">
          {steps[step].caption}
        </h2>
      ...
      <div className="p-9 flex flex-1 flex-col">{Body}</div>
    </div>
  );
};
